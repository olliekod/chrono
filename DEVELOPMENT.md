...
- FFmpeg: the app looks for `ffmpegfmpeg.exe` next to itself first and falls back to `PATH` (`FfmpegLocator`). Run `powershell -File scriptsetch-ffmpeg.ps1` once to download the build Chrono ships with (gyan.dev release-essentials, checksum-verified, git-ignored) into `client/ChronoRecorder/ffmpeg/`; the csproj copies it to the output. There is no `ffprobe` dependency: clip length and size come from parsing `ffmpeg -i` (`MediaInfoParser`). Also needs the Edge WebView2 runtime (`WebView2Runtime.EnsureInstalled` runs the bundled installer if it is missing).
