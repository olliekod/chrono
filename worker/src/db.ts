...
export async function insertClip(
  ...
  await db
    .prepare(
      `INSERT INTO clips (id, owner, filename, object_key, upload_id, size_bytes, duration_seconds, resolution, fps, bitrate_kbps)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    )
    .bind(id, clip.owner, clip.filename, objectKey, uploadId, clip.size, clip.duration, clip.resolution, clip.fps, clip.bitrate)
    .run();
}
